...
variable "cloudfront_geo_restriction_locations" {
  ...
  validation {
    error_message = "ISO 3166-1-alpha-2 code must contain at least 2 characters"
    condition     = alltrue([ for v in var.cloudfront_geo_restriction_locations : length(v) == 2 ])
  }
}
